-- The roads between two islands of doors, so the walk between them is on a road.
--
-- The walk takes the roads within three hundred metres of a door. A zone
-- with two clusters of doors further apart than that had the road between
-- them left out, and the walk crossed the gap in a straight line, through
-- the woods: twenty-five walkable zones still had a straight leg over three
-- hundred metres. Now, when the roads near the doors leave islands, the
-- roads in the box between each island and the nearest other are brought
-- in and the network is built again, up to three times; a box that is the
-- open countryside between two farms (over four thousand pieces) is left
-- alone, since that zone is driven. Every zone is walked again.
...
DECLARE
  ...
  n_comp INTEGER; n_more INTEGER; biggest BIGINT; gap RECORD; gb BOX; added INTEGER; bridge_pass INTEGER;
BEGIN
  ...
  FOR pass IN 1..3 LOOP
    ...
    FOR gap IN SELECT * FROM zw_gap LOOP
      gb := box(point(least(ST_X(gap.ag), ST_X(gap.bg)) * kx - 200, least(ST_Y(gap.ag), ST_Y(gap.bg)) * ky - 200),
                point(greatest(ST_X(gap.ag), ST_X(gap.bg)) * kx + 200, greatest(ST_Y(gap.ag), ST_Y(gap.bg)) * ky + 200));
      -- A box that is the countryside between two farms is left alone: the
      -- leg stays as the crow flies, and that zone is driven anyway.
...
-- Every zone is walked again on the joined roads.
UPDATE public.organizations SET roads_updated_at = now() WHERE roads_updated_at IS NOT NULL;
