-- A wider box between two islands of doors.
--
-- The box between two islands was two hundred metres about the straight
-- line between them, and the road joining them seldom runs straight: in
-- Aberdeen it goes round a golf course, and the walk still crossed the
-- gap as the crow flies. The box is now as wide about the line as half
-- the gap is long, and takes up to six thousand pieces. The roads are read
-- again with the trunk roads (US 40, which the doors along it are only
-- reached by), and that walks every zone again.
