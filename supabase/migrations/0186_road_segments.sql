-- The county's roads from OpenStreetMap, so the walks follow real roads.
--
-- USPS draws each carrier route's streets as islands: the road that joins
-- one cul-de-sac to the next is often not there, and the walk was left to
-- cross the gap in a straight line, through the woods. OpenStreetMap has
-- every road, footway and path. It is read a tile at a time in the
-- background (the osm_roads job), kept here in the same local metres as
-- the USPS lines, and zone_walk routes on it whenever a zone has any of
-- it about. Once the roads are in, every zone walked before them is
-- walked again, six every two minutes, so the database is never swamped.
...
CREATE INDEX IF NOT EXISTS road_segments_bbox_idx ON public.road_segments USING gist (bbox);
CREATE INDEX IF NOT EXISTS road_segments_tile_idx ON public.road_segments (organization_id, tile);

ALTER TABLE public.organizations ADD COLUMN IF NOT EXISTS roads_updated_at TIMESTAMPTZ;
ALTER TABLE public.hanger_zones ADD COLUMN IF NOT EXISTS walked_at TIMESTAMPTZ;
UPDATE public.hanger_zones SET walked_at = coalesce(updated_at, now()) WHERE walked_at IS NULL AND walk_path IS NOT NULL;

-- One tile's roads, as rows of [osm_id, highway, name, x1, y1, x2, y2] in
-- local metres; the tile's old rows go first when asked.
...
BEGIN
  ...
  RETURN jsonb_build_object('houses', n, 'pathKm', round((total_m / 1000.0)::numeric, 2), 'medianGapM', round(med::numeric, 1),
    'mode', the_mode, 'streets', true, 'roads', on_roads, 'minutes', minutes, 'strays', strays, 'edges', (SELECT count(*) FROM zw_edges));
END $$;

-- The zones walked before the roads came in, a few at a time.
CREATE OR REPLACE FUNCTION public.zones_rewalk_tick(n INTEGER DEFAULT 6)
RETURNS INTEGER LANGUAGE plpgsql SET search_path = public, pg_temp AS $$
DECLARE z RECORD; done INTEGER := 0; touched UUID[] := '{}';
BEGIN
  ...
  LOOP
    ...
    END;
    -- Walked or not, it is not tried again until the roads change.
  ...
  RETURN done;
END $$;
REVOKE ALL ON FUNCTION public.zones_rewalk_tick(INTEGER) FROM PUBLIC, anon, authenticated;

CREATE OR REPLACE FUNCTION public.zones_rewalk_pending(org UUID)
RETURNS INTEGER LANGUAGE sql STABLE SET search_path = public, pg_temp AS $$
  SELECT count(*)::integer FROM hanger_zones hz JOIN organizations o ON o.id = hz.organization_id
  WHERE hz.organization_id = org AND o.roads_updated_at IS NOT NULL AND (hz.walked_at IS NULL OR hz.walked_at < o.roads_updated_at);
$$;
