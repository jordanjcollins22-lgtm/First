-- Only the roads near the doors, and a re-walk tick that keeps to its minute.
--
-- A zone of six farms kilometres apart has a box that takes in every
-- lane of the countryside between them: tens of thousands of pieces,
-- more than can be noded before the statement timeout, and the re-walk
-- cron died on it every minute, losing the walks it had made. The walk
-- now takes only the roads within three hundred metres of a door, and
-- the tick starts no walk after twenty-five seconds.
...
BEGIN
  ...
  -- roads from OpenStreetMap, which meet exactly, are noded alone.
...
-- The zones walked before the roads came in, within a time budget: a
-- walk started late in the minute would be cut off by the statement
-- timeout, and a cut-off tick loses every walk it made.
...
BEGIN
  ...
  END LOOP;
  -- The zone summaries are redone once the queue is empty; the ten-minute
  -- refresh covers the meantime.
  IF done > 0 THEN
