...
export function readingSystemPrompt(brief: ReadingBrief): string {
  ...
  return [
    ...
    "",
    "author: the poster's first name as shown at the top of the post, beside their profile picture. Null only when no name is visible at all. Never a full name, and never anybody who only commented.",
    "",
    "summary: one short sentence, under 140 characters, saying what they want. Written for the person who has to answer it. Never empty. When the post is not a request, say what it is about instead.",
    "",
    ...
    "",
    "group: the group, neighbourhood or subreddit name written above the post, copied exactly as it appears. On Facebook it is the line above or beside the poster's name; on Nextdoor it is the neighbourhood. Null only when it is genuinely not in the picture. Do not use the poster's name, and do not guess it from what the post is about.",
    "",
