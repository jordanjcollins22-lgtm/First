...
import { discoverLayer, kickStep, selfBaseUrl, totals } from "@/lib/gis-import-run";
 ...
 */
export async function testGisConnection(serviceUrl: string): Promise<{ jobId: string; ok: boolean }> {
 ...
 */
export async function startGisImport(input: StartImportInput): Promise<{ jobId: string }> {
  const profile = await requireAccess();
  if (!env.cronSecret) {
...
export async function pauseGisImport(jobId: string) {
  await requireAccess();
  const admin = createAdminClient();
...
export async function resumeGisImport(jobId: string) {
  await requireAccess();
  const admin = createAdminClient();
 ...
 */
export async function runIntegrityChecks(): Promise<Json> {
  const profile = await requireAccess();
  const admin = createAdminClient();
  ...
  const { data: latest } = await admin
    ...
    .maybeSingle();
  if (latest) {
    await admin.from("gis_import_jobs").update({ after_totals: report, updated_at: new Date().toISOString() }).eq("id", latest.id);
  }
 ...
 */
export async function verifyKnownHouse(address: string): Promise<KnownHouseCheck> {
  await requireAccess();
  ...
  const { data, error } = await supabase
    .from("houses")
    .select(
      "id, address, parcel_id, gis_address, gis_matched_at, needs_review, property_id, property_events(kind), house_contacts(id)"
    )
