...
const clientJourney: JourneyTemplate = {
  ...
  steps: [
    ...
    },
    {
      ...
      nextSteps: ["proposal_generated"],
      estMinutes: null,
      notes: "The full Evaluator Journey — see that tab for the step-by-step.",
    },
    {
      stepKey: "proposal_generated",
      order: 4,
      label: "Proposal Generated",
      stepType: "human",
      roleLabel: "Admin/AM",
      inputs: ["Click Generate (or Regenerate) on the job page"],
      outputs: ["Scope + one total price snapshotted from the site map", "Shareable client link"],
      automations: [
        "Scope text auto-written per service from the site map's zones/answers",
        "Price snapshotted at generate time so it can't silently change once shared",
      ],
      nextSteps: ["proposal_view"],
      clicks: 1,
      estMinutes: 0.2,
      notes: "Regenerating keeps the same link but resets any prior client response.",
    },
    {
      stepKey: "proposal_view",
      order: 5,
      label: "Client Views Proposal",
      stepType: "customer_action",
      roleLabel: "Client",
      inputs: ["Opens the link an AM/admin sent them"],
    ...
    },
    {
      ...
      order: 6,
      label: "Client Accepts or Declines",
      stepType: "customer_action",
      roleLabel: "Client",
    ...
    },
    {
      ...
      clicks: 1,
      estMinutes: 0.2,
      notes: "Still manually editable/overridable here even after a proposal response sets it automatically.",
    },
    {
