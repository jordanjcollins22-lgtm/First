...
const clientJourney: JourneyTemplate = {
  ...
  steps: [
    ...
    },
    {
      ...
      manualInputs: 5,
      estMinutes: 1,
      notes: "The public /book page — reached by clicking an ad (the org's own link) or a specific evaluator/account manager's personal affiliate link.",
    },
    {
    ...
    },
    {
      ...
      outputs: ["Budget range attached to the lead — helps match the right materials"],
      nextSteps: ["public_schedule"],
      clicks: 1,
      estMinutes: 0.2,
    },
    {
      ...
      stepType: "customer_action",
      roleLabel: "Client",
      inputs: ["Date/time, from the evaluator's real availability"],
      outputs: ["Customer + property + job created, evaluation booked"],
      automations: [
        "Available times computed from weekly availability minus days off minus already-booked evaluations",
        "Books directly with the specific evaluator if their personal link was used; otherwise any available evaluator",
      ],
      nextSteps: ["lead_created"],
      clicks: 2,
      estMinutes: 0.5,
      notes: "The only channel where the evaluation gets scheduled and assigned in the same step, with no admin in the loop.",
    },
    {
