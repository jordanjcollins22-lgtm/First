...
import { AlertCircle, Check, FileText, Search, Trash2, Upload, User } from "lucide-react";
...
function UploadInvoice({ onSaved }: { onSaved: (invoice: ClientInvoice) => void }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card/60 p-3">
      ...
      {contact ? (
        <div className="mb-2 flex items-center justify-between gap-2 rounded-md border border-border bg-background px-2 py-1.5">
...
/** Finding the person this invoice went to. The same debounced search the
 * payments screen uses, so there is one way to find somebody rather than two. */
function ContactPicker({ onPick }: { onPick: (c: SearchableContact) => void }) {
    ...
    <div className="space-y-1.5">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Which contact is this invoice for?"
