...
export async function GET(request: NextRequest) {
  ...
  if (!isGmailConfigured) {
    return NextResponse.json({ ok: false, skipped: "Gmail sending is not set up. Add GMAIL_USER and GMAIL_APP_PASSWORD." });
  }
  ...
  const counts = { sent: 0, failed: 0, claimedElsewhere: 0 };
  ...
  for (const item of due) {
    ...
    if (sent.ok) {
      await admin.rpc("evaluation_sequence_sent", {
        ...
        p_thread_id: item.reply_thread_id ?? sent.messageId,
      });
      counts.sent += 1;
      log.info("evaluation.email.sent", { jobId: item.job_id, step: item.step, to: maskEmail(item.to_email) });
    } else {
  ...
  return NextResponse.json({ ok: true, due: due.length, ...counts });
}
